...
def test_pokemon_name_has_expected_id(pokemon_test_cases):
    ...
    assert response.status_code == 200
    assert response.json()["name"] == pokemon_name, f"Expected name {pokemon_name} but got {response.json()['name']} for ID {pokemon_id}"
...
def test_pokemon_id_has_expected_name(pokemon_test_cases):
    ...
    response = requests.get(f"https://pokeapi.co/api/v2/pokemon/{pokemon_name}")
    assert response.status_code == 200
    assert response.json()["id"] == pokemon_id, f"Expected ID {pokemon_id} but got {response.json()['id']} for name {pokemon_name}"